...
example, we will use an existing [Unikraft](unikraft.org) Unikernel
...
In order to append `urunc` annotations in an exisitng Unikraft OCI image,
...
- We want to use a Unikraft unikernel that will execute on top of Qemu over x86
  architecture.
...
In the case of the Dockerfile-like syntax, we need to manually specify the
`urunc` annotations, using the respective labels. The above `bunnyfile` is equal
to the following `Containerfile`:
...
In the above file:
- We directly the `unikraft.org/nginx:1.15` as the base for our OCI image.
