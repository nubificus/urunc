...
`urunc` currently supports running applications as unikernels. Unikernels are
specialized, minimalistic operating systems constructed to run a single
application. By compiling only the necessary components of an OS into the final
image, unikernels offer improved performance, security, and smaller footprints
compared to traditional OS-based virtual machines.

Some of the prominent frameworks supported include:
...
`urunc` currently supports `Unikraft` and `Rumprun`, while support for
additional unikernel frameworks is [in progress](LINK TO ROADMAP).
