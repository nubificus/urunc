...
func (u Unikontainer) joinSandboxNetNs() error {
	...
	//    unikernel dies/exits the namesapce will also die, since there will
	//    not be any process in that namespace. Therefore, the clenaup will
	...
	if err != nil {
		return fmt.Errorf("Error joining namesapce: %w", err)
	}
...
// FormatNsenterComm encodes namespace info in netlink binary format
...
func (u *Unikontainer) FormatNsenterComm() (rdr io.Reader, Err error) {
	...
	const numNS = 8
	var writePaths bool = false
	var writeFlags bool = false
	var cloneFlags int
	...
	for _, ns := range u.Spec.Linux.Namespaces {
		// If the path is empty, the we have to create it.
	...
	// we cna set up the uid/gid mappings only once in a user namespace.
